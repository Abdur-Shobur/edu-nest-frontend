...
import { useUserDeleteMutation, useUserStatusMutation } from './api-slice';
...
const DropDownAction = ({ item }: { item: IUser }) => {
	const [mutation, { isLoading }] = useUserDeleteMutation();
	...
	return (
		<DropdownMenu>
			...
			<DropdownMenuContent align="end" className="w-56">
				...
				{/* STATUS ACTIVE   */}
				{item.status !== IUserStatus.Active && (
					<ApiStatusDropdownHandler
						data={{ id: item.id, status: IUserStatus.Active }}
				...
				{/* STATUS INACTIVE   */}
				{item.status !== IUserStatus.Inactive && (
					<ApiStatusDropdownHandler
						data={{ id: item.id, status: IUserStatus.Inactive }}
						mutation={mutationStatus}
						isLoading={isLoadingStatus}
						text="Set Inactive"
						icon="CircleOff"
				...
				{/* STATUS PENDING   */}
				{item.status !== IUserStatus.Pending && (
					<ApiStatusDropdownHandler
						data={{ id: item.id, status: IUserStatus.Pending }}
						mutation={mutationStatus}
						isLoading={isLoadingStatus}
						text="Set Pending"
						icon="Clock"
				...
				<DropdownMenuSeparator />
				{/* STATUS BLOCKED   */}
				{item.status !== IUserStatus.Blocked && (
					<ApiStatusDropdownHandler
						data={{ id: item.id, status: IUserStatus.Blocked }}
						mutation={mutationStatus}
						isLoading={isLoadingStatus}
						text="Set Blocked"
						icon="CircleOff"
				...
				{/* STATUS DELETED   */}
				{item.status !== IUserStatus.Trashed && (
					<ApiStatusDropdownHandler
						data={{ id: item.id, status: IUserStatus.Trashed }}
						mutation={mutationStatus}
						isLoading={isLoadingStatus}
						text="Move to Trash"
						icon="Trash"
