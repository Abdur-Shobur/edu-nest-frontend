...
import { ISettingsStatus } from './type';
...
const schema = z.object({
	...
	status: z.enum([ISettingsStatus.Active, ISettingsStatus.Inactive]),
});
		...
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger asChild>
				<Button size="lg" variant="brand">
					...
					<span className="hidden md:inline">Create Settings</span>
				</Button>
			</DialogTrigger>

			<DialogContent className="sm:max-w-[500px] overflow-y-scroll max-h-[90vh]">
				<DialogHeader>
					<DialogTitle>Create Settings</DialogTitle>
...
const FORM = ({ setOpen }: { setOpen: Dispatch<SetStateAction<boolean>> }) => {
	const [store, { isLoading }] = useSettingsStoreMutation();
					...
					render={({ field }) => (
						<FormItem>
							...
							<Select onValueChange={field.onChange} defaultValue={field.value}>
								...
								<SelectContent>
									<SelectItem value={ISettingsStatus.Active}>Active</SelectItem>
									<SelectItem value={ISettingsStatus.Inactive}>
										Inactive
									</SelectItem>
								</SelectContent>
							</Select>
