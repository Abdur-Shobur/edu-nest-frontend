						...
						{data?.length === 0 ? (
							...
							data?.map((item, i) => (
								<TableRow key={item.id}>
									...
									<TableCell className="py-2">
										<Badge variant="info">{item.sectionsKey}</Badge>
									</TableCell>
...
const DropDownAction = ({ item }: { item: ICMSSections }) => {
	const [mutation, { isLoading }] = useCMSSectionsDeleteMutation();
