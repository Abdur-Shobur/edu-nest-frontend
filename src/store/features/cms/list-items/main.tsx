						...
						{data?.length === 0 ? (
							...
							data?.map((item, i) => (
								<TableRow key={item.id}>
									...
									<TableCell className="py-2">
										<Badge variant="info">{item.listKey}</Badge>
									</TableCell>
...
const DropDownAction = ({ item }: { item: ICMSListItems }) => {
	const [mutation, { isLoading }] = useCMSListItemsDeleteMutation();
